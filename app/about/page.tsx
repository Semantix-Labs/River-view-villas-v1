      ...
      <section className="section-pad" style={{ background: 'var(--cream)' }}>
        <div className="container-rv">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '80px', alignItems: 'center' }}>
            ...
            <FadeIn delay={0.2}>
              <div style={{ background: 'linear-gradient(145deg, #4A6670 0%, #2C2016 100%)', borderRadius: '4px', padding: '60px 48px', position: 'relative' }}>
                <div style={{ position: 'absolute', top: '24px', right: '24px', width: '60px', height: '60px', borderRadius: '50%', border: '1px solid rgba(201,169,110,0.25)' }} />
                <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '28px', fontStyle: 'italic', color: 'var(--gold-light)', lineHeight: 1.5, marginBottom: '32px' }}>
                ...
                <div style={{ display: 'flex', gap: '40px', flexWrap: 'wrap' }}>
                  {[{ n: '2005', l: 'Founded' }, { n: '20+', l: 'Years' }, { n: '5', l: 'Villas' }].map(s => (
                    <div key={s.l}>
                      ...
                      <p style={{ color: 'rgba(247,243,238,0.5)', fontSize: '11px', letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: '6px' }}>{s.l}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {team.map((member, i) => (
              <FadeIn key={member.name} delay={i * 0.1}>
                <div className="card" style={{ padding: '36px', height: '100%' }}>
                  <div style={{ width: '52px', height: '52px', borderRadius: '50%', background: 'linear-gradient(135deg, var(--gold) 0%, var(--gold-light) 100%)', marginBottom: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '22px', fontWeight: 600, color: 'var(--deep)' }}>{member.name.charAt(0)}</span>
                  </div>
                  ...
                  <p style={{ color: 'var(--text-light)', fontSize: '12px', marginBottom: '16px', fontStyle: 'italic' }}>{member.credentials}</p>
